...
/* =====================================================
  app.js — Viewer (fixed: PIN order, mobile PIN focus, riskline hydration,
  hover time label, click/drag seek on mobile & desktop, webcam visibility,
  and reliable overlay placement + spacing knobs)
===================================================== */
...
const INSTANCE_ID = ENV.INSTANCE_ID || null; // optional
const slugFromQS = (() => {
...
// topDock (for mobile offer docking)
...
function playActiveMusicForState(){ if(!musicEnabled) return; const inOffer=document.body.classList.contains('offer-mode'); const target=inOffer?endMusic:bgMusic; try{ target&&target.play().catch(()=>{});}catch{} }
...
function guessStorageKeyVariants(id,bucket){ const bases=[String(id)]; const withFolders=p=>[p,`slides/${p}`,`presentation/${p}`,`uploads/${p}`]; const exts=['','.mp4','.webm','.m4v','.mov']; const out=[]; for(const base of bases){ for(const ext of exts){ for(const v of withFolders(base+ext)) out.push({bucket, path:v}); } } return out; }
async function resolveIdToURL(id, preferredBucket){ if(!SB||!id) return null; const TABLES=['videos','hello_videos','hello_assets_videos','slides','hello_slides','media','presentation_videos']; for(const table of TABLES){ try{ const {data,error}=await SB.from(table).select('*').eq('id',id).limit(1); if(!error&&data&&data[0]){ const row=data[0]; if(row.storage_bucket&&(row.storage_key||row.storage_path)) return await storageSignedOrPublicURL(row.storage_bucket,row.storage_key||row.storage_path); if(row.bucket&&(row.key||row.path)) return await storageSignedOrPublicURL(row.bucket,row.key||row.path); if(row.storage_path) return await storageSignedOrPublicURL(preferredBucket||SLIDES_BUCKET,row.storage_path); return row.public_url||row.file_url||row.url||null; } }catch{} }
  if(isUUID(id)){
    ...
    for(const g of guesses){ const u=await storageSignedOrPublicURL(g.bucket,g.path); if(u) return u; }
  }
  return null;
}
...
function injectHighlights(resolvedText, arr){ if(!resolvedText) return ''; let html=String(resolvedText); (arr||[]).forEach(h=>{ const safe=h.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'); const rx=new RegExp(`(\\b${safe}\\b)`,'ig'); html=html.replace(rx,'<span class="ink ink--hot">$1<\/span>'); }); return html; }
...
async function hydrateFromCloud(){
  ...
  try{
    ...
    const [{ data: leads }, { data: presArr }] = await Promise.all([leadQ, presQ]);
    ...
    function deepCandidates(obj, kind, buckets, maxDepth=4, pathSeen=new WeakSet()){
      ...
      function push(val, srcBucket){ if(!val) return; if(typeof val==='string'){ if(looksLikeURL(val)) out.push({url:val}); else if(isUUID(val)) out.push({id:val}); else if(looksLikePath(val)) out.push({bucket:srcBucket, path:val}); } else if(typeof val==='object'){ const b=val.bucket||srcBucket; if(val.id) out.push({id:val.id}); if(val.bucket&&(val.key||val.path)) out.push({bucket:val.bucket, path:val.key||val.path}); if(val.key) out.push({bucket:b, path:val.key}); if(val.path) out.push({bucket:b, path:val.path}); if(val.url) out.push({url:val.url}); }}
      function walk(node, depth, defaultBucket){ if(!node||typeof node!=='object'||depth>maxDepth||pathSeen.has(node)) return; pathSeen.add(node); for(const k of Object.keys(node)){ const v=node[k]; const key=k.toLowerCase(); if(ban && ban.test(key)) continue; const isSlidesy=/slides?|deck|presentation|main(video)?|primary(video)?|intro(video)?|video$/.test(key); const isWebcamy=/(web)?cam(video)?|primarycam/.test(key); if((kind==='slides'&&isSlidesy)||(kind==='webcam'&&isWebcamy)){ push(v, kind==='slides'?buckets.slides:buckets.cam); if(typeof v==='object') walk(v, depth+1, kind==='slides'?buckets.slides:buckets.cam); continue; } if(typeof v==='string'){ if(looksLikeURL(v)||isUUID(v)||looksLikePath(v)){ push(v, kind==='slides'?buckets.slides:buckets.cam); continue; }} if(typeof v==='object') walk(v, depth+1, defaultBucket); } }
    ...
    const vOver = (link?.overrides?.videos)||{}; const vDef = (pres?.defaults?.videos)||(pres?.videos)||(pres?.media)||{};
    ...
    const slidesCandidates = [ ...gatherVideoCandidates(vOver,'slides',BKT), ...gatherRootVideoCandidates(link?.overrides,'slides',BKT), ...gatherVideoCandidates(vDef,'slides',BKT), ...gatherRootVideoCandidates(pres,'slides',BKT), ...deepCandidates(link?.overrides,'slides',BKT), ...deepCandidates(pres,'slides',BKT), legacySlidesUrlOverride?{url:legacySlidesUrlOverride}:null, legacySlidesUrlDefault?{url:legacySlidesUrlDefault}:null ];
    const webcamCandidates = [ ...gatherVideoCandidates(vOver,'webcam',BKT), ...gatherRootVideoCandidates(link?.overrides,'webcam',BKT), ...gatherVideoCandidates(vDef,'webcam',BKT), ...gatherRootVideoCandidates(pres,'webcam',BKT), ...deepCandidates(link?.overrides,'webcam',BKT), ...deepCandidates(pres,'webcam',BKT), legacyWebcamUrlOverride?{url:legacyWebcamUrlOverride}:null, legacyWebcamUrlDefault?{url:legacyWebcamUrlDefault}:null ];
    ...
    if(!handUrl && legacyHandId){
      try{ const {data}=await SB.from('hello_assets_handcams').select('*').eq('id', legacyHandId).limit(1).single();
        if(data){ if(data.storage_bucket&&(data.storage_key||data.storage_path)) handUrl=await storageSignedOrPublicURL(data.storage_bucket, data.storage_key||data.storage_path);
          else if(data.bucket&&(data.key||data.path)) handUrl=await storageSignedOrPublicURL(data.bucket, data.key||data.path);
          else if(data.storage_path) handUrl=await storageSignedOrPublicURL(BKT.hand, data.storage_path);
          else handUrl=data.file_url||data.public_url||data.url||null; }
      }catch{}
    }

    // CTA merge & resolve tags
    ...
    // Riskline hydration (multiple possible targets)
    const riskTargets = [
...
function showStage(){ stage.classList.add('show'); orbitReplay.classList.remove('show'); skipIntroBtn.classList.remove('show'); skipIntroBtn.classList.add('hidden'); try{ mainVideo.play().catch(()=>{});}catch{} enterNormalMode(); if(bgMusic){ try{ setVolumeSafe(bgMusic, MUSIC.MAIN_VOL); musicEnabled && bgMusic.play().catch(()=>{});}catch{} } stabilizeIntrigue(800); // important for correct positions
  // ensure scrub overlay positions after the stage is visible
  try{ window.__placeScrubOverlay && window.__placeScrubOverlay(); }catch{}
}
...
let resizeRaf=0; window.addEventListener('resize', ()=>{ if(resizeRaf) cancelAnimationFrame(resizeRaf); resizeRaf=requestAnimationFrame(()=>{ positionIntrigueBetween(); stabilizeIntrigue(300); try{ window.__placeScrubOverlay && window.__placeScrubOverlay(); }catch{} }); });
let roPending=false; const ro=new ResizeObserver(()=>{ if(roPending) return; roPending=true; requestAnimationFrame(()=>{ roPending=false; positionIntrigueBetween(); try{ window.__placeScrubOverlay && window.__placeScrubOverlay(); }catch{} }); }); if(nav) ro.observe(nav); if(playerEl) ro.observe(playerEl);
...
function syncPrimaryWebcamToMain({force=false, forceWhenMetaReady=false}={}){ const tMain=mainVideo.currentTime||0; const applySync=()=>{ const tCam=computeCamTimeForMain(tMain); if(tCam==null) return false; try{ webcamVideo.currentTime=tCam; }catch{} return true; };
  if(isFinite(mainVideo.duration)&&isFinite(webcamVideo.duration)) return applySync();
  if(force||forceWhenMetaReady){ const onMainMeta=()=>{ mainVideo.removeEventListener('loadedmetadata', onMainMeta); maybeDo(); }; const onCamMeta=()=>{ webcamVideo.removeEventListener('loadedmetadata', onCamMeta); maybeDo(); }; function maybeDo(){ if(isFinite(mainVideo.duration)&&isFinite(webcamVideo.duration)) applySync(); } mainVideo.addEventListener('loadedmetadata', onMainMeta); webcamVideo.addEventListener('loadedmetadata', onCamMeta); }
  return false;
}
...
mainVideo.addEventListener('loadeddata',()=>{ updateMainUI(); syncPrimaryWebcamToMain({forceWhenMetaReady:true}); stabilizeIntrigue(600); try{ window.__placeScrubOverlay && window.__placeScrubOverlay(); }catch{} });
...
/* ===================== Progress + time-left ===================== */
function fmtTimeLeft(totalSec, cur){ const remain=Math.max(0, Math.floor(totalSec-cur)); const m=Math.floor(remain/60); const s=remain%60; return `${m}:${String(s).padStart(2,'0')}`; }
...
function applySeekLockUI(){ if(isSeekEnabled()){ progress.classList.remove('disabled'); progress.setAttribute('aria-disabled','false'); } else { progress.classList.add('disabled'); progress.setAttribute('aria-disabled','true'); }
  if(goEndWrap){ goEndWrap.classList.add('visible'); goEndWrap.style.display='block'; const inEnd=playerEl.classList.contains('content-hidden'); const btn=goEndWrap.querySelector('.go-end-btn'); if(btn) btn.style.display=(isSeekEnabled() && !inEnd)?'':'none'; }
  try{ window.__updateScrubOverlay && window.__updateScrubOverlay(); }catch{}
}
...
function seekFromEvent(e){ if(!isSeekEnabled()) return; if(!isFinite(mainVideo.duration)||mainVideo.duration<=0) return; const rect=progress.getBoundingClientRect(); const clientX=(e.clientX!==undefined)?e.clientX:(e.touches&&e.touches[0]?e.touches[0].clientX:0); const x=Math.min(Math.max(0, clientX-rect.left), rect.width); const ratio = rect.width ? x/rect.width : 0; const newTime=ratio*mainVideo.duration; setMainTimeAndHardSync(newTime); }
...
/* ===================== Preview loop ===================== */
let previewLoopActive=true; const PREVIEW_LOOP_EPS=.12;
...
/* ===================== PIN overlay logic (fixed order) ===================== */
(function setupPin(){
  const pinOverlay=document.getElementById('pinOverlay');
  const pinBoxes=document.getElementById('pinBoxes');
  const pinError=document.getElementById('pinError');
  const digits=[]; const cells=Array.from(pinBoxes?.querySelectorAll('.pin-digit')||[]);

  // Force visual LTR and stable order (prevents reversed feel on some mobiles)
  if(pinBoxes){ pinBoxes.style.direction='ltr'; pinBoxes.style.flexDirection='row'; pinBoxes.style.gap = pinBoxes.style.gap || '10px'; }
  cells.forEach(c=>{ c.style.order='0'; });
  ...
  pinOverlay && pinOverlay.appendChild(pinInput);

  function focusPinSoon(){ setTimeout(()=>{ try{ pinInput.focus(); }catch{} },0); }
  pinOverlay && pinOverlay.addEventListener('click', focusPinSoon);

  const normalizePin = s => String(s||'').replace(/\D/g,'');
  ...
  function clearDigits(){ digits.length=0; render(); pinInput.value=''; }
  ...
  function acceptPin(){ slideOutOverlay(); try{ pinInput.blur(); }catch{} (async()=>{ await HYDRATE_DONE; if(HYDR.notFound){ showNotFound(); return; } runSequence(); })(); }

  function checkIfReady(){
    if(!PIN_REQUIRED){ acceptPin(); return; }
    if(digits.length!==4) return;
    const attempt = normalizePin(digits.join(''));
    const expected = normalizePin(PIN_EXPECTED);
    if(attempt===expected || attempt.split('').reverse().join('')===expected){ // accept reversed too (safety)
      acceptPin();
    } else {
      flashError(); setTimeout(clearDigits,180);
    }
  }

  pinInput.addEventListener('input', ()=>{
    const v = normalizePin(pinInput.value).slice(0,4);
    digits.length=0; for(const ch of v) digits.push(ch);
    render(); if(digits.length===4) checkIfReady();
  });
  ...
  render(); focusPinSoon();
  (async function waitForPinDecision(){ await HYDRATE_DONE; if(!PIN_REQUIRED){ acceptPin(); } })();
})();
...
/* ===================== Go-to-End + mobile docking ===================== */
function setupGoToEndButton(){ if(!playerEl) return; goEndWrap=document.createElement('div'); goEndWrap.className='go-end-wrap'; goEndWrap.style.display='block'; goEndBtn=document.createElement('button'); goEndBtn.id='goToEndBtn'; goEndBtn.type='button'; goEndBtn.className='go-end-btn'; goEndBtn.setAttribute('aria-label','Go to the end'); goEndBtn.innerHTML=`<i class="ri-skip-forward-fill" aria-hidden="true"></i><span class="go-end-text">Go to the end</span>`; goEndWrap.appendChild(goEndBtn); playerEl.parentNode.insertBefore(goEndWrap, playerEl.nextSibling); goEndBtn.addEventListener('click',()=>{ const btn=goEndWrap.querySelector('.go-end-btn'); if(btn) btn.style.display='none'; goToEndNow(); }); const btn=goEndWrap.querySelector('.go-end-btn'); if(btn) btn.style.display='none'; window.__belowPlayerArea = goEndWrap;

  function applyWebcamTimelineGapInline(){ const isSmall=window.matchMedia('(max-width: 768px)').matches; if(!webcamWrap) return; if(isSmall){ const cs=getComputedStyle(document.documentElement); const gap=(cs.getPropertyValue('--webcam-timeline-gap')||'12px').trim(); webcamWrap.style.marginBottom = gap || '12px'; } else { webcamWrap.style.marginBottom = ''; } }

  function relocateWebcamForSmall(){ const isSmall=window.matchMedia('(max-width: 768px)').matches; if(!webcamWrap) return; if(isSmall && document.body.classList.contains('offer-mode') && topDock){ if(webcamWrap.parentNode!==topDock) topDock.appendChild(webcamWrap); document.body.dataset.webcamPosition='below'; requestAnimationFrame(()=>{ positionOrbitOnRim(315); applyWebcamTimelineGapInline(); }); return; } const area=window.__belowPlayerArea; if(isSmall && area){ if(webcamWrap.parentNode!==area) area.appendChild(webcamWrap); document.body.dataset.webcamPosition='below'; requestAnimationFrame(()=>{ positionOrbitOnRim(315); applyWebcamTimelineGapInline(); }); return; } if(webcamWrap.parentNode!==playerEl) playerEl.appendChild(webcamWrap); delete document.body.dataset.webcamPosition; requestAnimationFrame(()=>{ positionOrbitOnRim(315); applyWebcamTimelineGapInline(); }); }
...
(function setupHoverTimeLabel(){
  ...
  window.addEventListener('orientationchange', ()=>{ setTimeout(placeOverlay, 100); });
  document.addEventListener('visibilitychange', ()=>{ setTimeout(placeOverlay, 50); });
  window.__placeScrubOverlay = placeOverlay; // expose for stage/hydration
