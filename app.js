/* =====================================================
  ...
  "Deep-diagnostics" build: Only added console logs + tiny helpers
  to debug mobile PIN issues. **NO behavior/logic changes.**

  What’s logged:
  - UA / platform snapshot
  - Hydration start/finish + resolved CTA (incl. riskline)
  - PIN overlay lifecycle: focus, input, keydown, compare, error
  - Attempt vs expected (both strings and char codes)
...
(function bootDiagnostics(){
  try {
    const caps = {
      ...
      pointerEnabled: !!(window.navigator && window.navigator.pointerEnabled),
      msPointerEnabled: !!(window.navigator && window.navigator.msPointerEnabled),
      cookieEnabled: navigator.cookieEnabled,
    };
    console.info('[DBG] boot caps', caps);
  } catch (e) { console.warn('[DBG] boot caps fail', e); }
})();
...
async function resolveIdToURL(id, preferredBucket) {
  ...
  for (const table of TABLES) {
    try { const { data, error } = await SB.from(table).select('*').eq('id', id).limit(1); if (!error && data && data[0]) { const row = data[0]; if (row.storage_bucket && (row.storage_key || row.storage_path)) return await storageSignedOrPublicURL(row.storage_bucket, row.storage_key || row.storage_path); if (row.bucket && (row.key || row.path)) return await storageSignedOrPublicURL(row.bucket, row.key || row.path); if (row.storage_path) return await storageSignedOrPublicURL(preferredBucket || SLIDES_BUCKET, row.storage_path); return row.public_url || row.file_url || row.url || null; } } catch { }
  }
  if (isUUID(id)) { const guesses = guessStorageKeyVariants(id, preferredBucket || SLIDES_BUCKET); for (const g of guesses) { const u = await storageSignedOrPublicURL(g.bucket, g.path); if (u) return u; } }
  return null;
}

/* ---------- ID → URL resolution helpers used in hydrate ---------- */
...
async function hydrateFromCloud() {
  HYDR.started = true;
  console.log('[DBG] hydrateFromCloud: start', { slugFromQS, hasSB: !!SB });
  try {
    if (!SB || !slugFromQS) {
      console.warn('[DBG] hydrateFromCloud: missing SB or slug', { SB: !!SB, slugFromQS });
    ...
    function gatherRootVideoCandidates(obj, kind, buckets) {
      ...
      const keys = [ `${kind}Key`, `${kind}_key`, `${kind}Path`, `${kind}_path`, `${kind}Url`, `${kind}_url`, `${kind}URL`, `${kind}Id`, `${kind}_id`, `${kind}ID`, kind === 'slides' ? 'deckKey' : 'camKey', kind === 'slides' ? 'deckUrl' : 'camUrl', kind === 'slides' ? 'deckId' : 'camId', 'videoUrl', 'video_key', 'videoKey', 'video_path', 'videoPath', 'videoId', 'mainVideo', 'primaryVideo' ];
      const out = [];
      for (const k of keys) { const v = obj?.[k]; if (!v) continue; if (k.toLowerCase().endsWith('id')) out.push({ id: v }); else if (k.toLowerCase().includes('url')) out.push({ url: v }); else out.push({ bucket: bkt, path: v }); }
      return out;
    }
    ...
    console.debug('[DBG] media resolved', { slidesUrl, webcamUrl, handUrl });
    ...
    console.debug('[DBG] ctaResolved', ctaResolved);
    ...
    const riskTargets = [ document.getElementById('ctaRiskline'), document.querySelector('.cta-riskline'), document.querySelector('[data-cta="riskline"]'), document.querySelector('#endCta .riskline, .end-cta .riskline') ].filter(Boolean);
    ...
    } catch (e) { console.warn('[DBG] video source attach error', e); }
    ...
    console.log('[DBG] PIN config', { PIN_REQUIRED, PIN_EXPECTED, len: PIN_EXPECTED.length });
    ...
    console.log('[DBG] hydrateFromCloud: done');
  } catch (err) {
...
/* ===================== Seek gating + hover insight (unchanged) ===================== */
...
function applySeekLockUI() { if (isSeekEnabled()) { progress.classList.remove('disabled'); progress.setAttribute('aria-disabled', 'false'); } else { progress.classList.add('disabled'); progress.setAttribute('aria-disabled', 'true'); } if (goEndWrap) { goEndWrap.classList.add('visible'); const inEnd = playerEl.classList.contains('content-hidden'); const btn = goEndWrap.querySelector('.go-end-btn'); if (btn) btn.style.display = (isSeekEnabled() && !inEnd) ? '' : 'none'; } }
...
/* ===================== Slide mechanics / orbit (unchanged) ===================== */
...
(function setupPin() {
  ...
  console.log('[DBG] setupPin: overlay', !!pinOverlay, 'boxes', !!pinBoxes, 'cells', cells.length);
  ...
  console.log('[DBG] setupPin: hidden input created');

  function focusPinInputSoon() { setTimeout(() => { try { pinInput.focus(); console.log('[DBG] pinInput.focus() ok'); } catch (e) { console.warn('[DBG] pinInput.focus() fail', e); } }, 0); }
  ...
  function flashError() { console.warn('[DBG] flashError'); pinError.classList.add('show'); cells.forEach(c => { c.classList.add('pulse'); setTimeout(() => c.classList.remove('pulse'), 900); }); setTimeout(() => pinError.classList.remove('show'), 1400); }
  function clearDigits() { console.log('[DBG] clearDigits'); digits.length = 0; render(); }
  function playPinSound() { if (!sfxPin) return; try { sfxPin.currentTime = 0; sfxPin.play().catch(() => { }); } catch { } }
  function slideOutOverlay() { console.log('[DBG] slideOutOverlay'); pinOverlay.classList.add('fade-out'); setTimeout(() => { pinOverlay.style.display = 'none'; }, 380); }

  const charCodes = (s) => Array.from(String(s||'')).map(ch => ch.charCodeAt(0));
  ...
  pinInput.addEventListener('input', () => {
    ...
    render();
    console.log('[DBG] pinInput[input]', { raw, filtered, len: filtered.length, digits: digits.join(''), digitCodes: charCodes(digits.join('')) });
    if (digits.length === 4) checkIfReady();
  });

  async function startAfterPin() {
    console.log('[DBG] startAfterPin()');
    slideOutOverlay();
    try { pinInput.blur(); } catch { }
    await HYDRATE_DONE;
    if (HYDR.notFound) { console.warn('[DBG] startAfterPin: HYDR.notFound'); showNotFound(); return; }
    runSequence();
  }

  function checkIfReady() {
    console.log('[DBG] checkIfReady()', { PIN_REQUIRED, PIN_EXPECTED });
    if (!PIN_REQUIRED) { startAfterPin(); return; }
    if (digits.length !== 4) return;
    const attempt = digits.join('');
    const expected = String(PIN_EXPECTED);
    const out = { attempt, expected, attemptCodes: charCodes(attempt), expectedCodes: charCodes(expected) };
    const ok = (String(attempt) === String(expected));
    console.log('[DBG] PIN compare', { ...out, ok });
    if (ok) { startAfterPin(); } else { flashError(); setTimeout(clearDigits, 180); }
  }

  function onKey(e) {
    if (!pinOverlay || pinOverlay.style.display === 'none') return;
    const k = e.key;
    console.log('doc.keydown', { key: k, code: e.code, which: e.which, keyCode: e.keyCode, ctrl: e.ctrlKey, meta: e.metaKey });
    if (k >= '0' && k <= '9') { if (digits.length < 4) { digits.push(k); render(); playPinSound(); console.log('[DBG] keydown add digit', { k, digits: digits.join('') }); if (digits.length === 4) checkIfReady(); } e.preventDefault(); return; }
    if (k === 'Backspace' || k === 'Delete') { if (digits.length > 0) { digits.pop(); render(); playPinSound(); console.log('[DBG] keydown backspace', { digits: digits.join('') }); } e.preventDefault(); return; }
  }
  ...
  (async function waitForPinDecision() {
    await HYDRATE_DONE;
    if (!PIN_REQUIRED) { console.log('[DBG] PIN not required; autostart'); startAfterPin(); }
    else { console.log('[DBG] PIN required; waiting for input'); }
  })();
})();
...
/* ===================== Setup Go-to-End container & mobile docking (unchanged) ===================== */
...
/* ===================== HOVER/CURSOR TIME LABEL (unchanged) ===================== */
