/* =====================================================
  ...
  - **Mobile PIN always works** even when mixing inputs (phone keyboard + desktop DevTools typing).
    • Single source of truth is the hidden <input> value (no desync with a separate digits array).
    • Normalizes Arabic‑Indic, Eastern Arabic‑Indic and FULLWIDTH numerals to ASCII 0–9.
    • Ignores non-digit characters (spaces, hyphens, OTP autofill junk).
    • Max length is generous before hydration, then tightened to the exact PIN length after hydration.
  - **Riskline** under the offer buttons is hydrated from DB reliably.
  - Keeps previous timeline hover label + bigger touch hitbox and all prior features intact.

  NOTE: Everything outside the requested areas is left as-is.
===================================================== */
...
let __PIN_EXPECTED_LEN = 4; // updated after hydration
...
// Top dock used in offer mode on mobile
...
function setVolumeSafe(a,v){ try{ if(a) a.volume = clamp01(v); }catch{} }
function fadeAudio(el, from, to, ms){ return new Promise(res=>{ if(!el) return res(); from=clamp01(from); to=clamp01(to); if(ms<=0||Math.abs(to-from)<.001){ setVolumeSafe(el,to); return res(); } const t0=performance.now(); setVolumeSafe(el,from); const step=(now)=>{ const r=clamp01((now-t0)/ms); setVolumeSafe(el, from+(to-from)*r); if(r>=1) return res(); requestAnimationFrame(step); }; requestAnimationFrame(step); }); }
let musicEnabled=true; function pauseBothMusic(){ try{bgMusic&&bgMusic.pause()}catch{}; try{endMusic&&endMusic.pause()}catch{} }
function playActiveMusicForState(){ if(!musicEnabled) return; const inOffer=document.body.classList.contains('offer-mode'); const t=inOffer?endMusic:bgMusic; try{ t&&t.play().catch(()=>{}) }catch{} }
function primeEndMusicOnce(){ if(!MUSIC.PRIME_ON_UNMUTE||!endMusic) return; try{ const m=endMusic.muted; endMusic.muted=true; endMusic.currentTime=0; endMusic.play().then(()=>{ endMusic.pause(); endMusic.currentTime=0; endMusic.muted=m; }).catch(()=>{ endMusic.muted=m; }); }catch{} }
...
/* ===================== Expiry countdown ===================== */
const STORAGE_KEY_EXPIRY='pv_link_expiry_ts'; let expiryTS=Number(localStorage.getItem(STORAGE_KEY_EXPIRY)); const nowTS=Date.now(); if(!expiryTS||expiryTS<nowTS){ expiryTS=nowTS+EXPIRY_DAYS*24*60*60*1000; localStorage.setItem(STORAGE_KEY_EXPIRY,String(expiryTS)); }
...
function tick(){ const remain=expiryTS-Date.now(); const t=fmt(remain); if(navCountdown) navCountdown.textContent=t; if(separateCountdown) separateCountdown.textContent=t; if(ctaCountdownEl) ctaCountdownEl.textContent=t; if(remain<=0){ try{ mainVideo.pause(); }catch{} } }
...
/* ===================== Storage helpers (unchanged core) ===================== */
...
async function resolveIdToURL(id,preferredBucket){ if(!SB||!id) return null; const TABLES=['videos','hello_videos','hello_assets_videos','slides','hello_slides','media','presentation_videos']; for(const t of TABLES){ try{ const {data,error}=await SB.from(t).select('*').eq('id',id).limit(1); if(!error&&data&&data[0]){ const row=data[0]; if(row.storage_bucket&&(row.storage_key||row.storage_path)) return await storageSignedOrPublicURL(row.storage_bucket,row.storage_key||row.storage_path); if(row.bucket&&(row.key||row.path)) return await storageSignedOrPublicURL(row.bucket,row.key||row.path); if(row.storage_path) return await storageSignedOrPublicURL(preferredBucket||SLIDES_BUCKET,row.storage_path); return row.public_url||row.file_url||row.url||null; } }catch{} } if(isUUID(id)){ const bases=[id,`slides/${id}`,`presentation/${id}`,`uploads/${id}`]; const exts=['','.mp4','.webm','.m4v','.mov']; for(const b of bases) for(const e of exts){ const u=await storageSignedOrPublicURL(preferredBucket||SLIDES_BUCKET,`${b}${e}`); if(u) return u; } } return null; }
async function pickFirstVideoURLAsync(candidates,fallbackBucketIfId){ for(const c of (candidates||[])){ if(!c) continue; if(c.id){ const byId=await resolveIdToURL(c.id,fallbackBucketIfId); if(byId) return byId; } if(c.bucket&&c.path){ const u=await storageSignedOrPublicURL(c.bucket,c.path); if(u) return u; } if(c.url){ const p=partsFromPublicURL(c.url); if(p){ const u=await storageSignedOrPublicURL(p.bucket,p.path); if(u) return u; } return c.url; } } return null; }
...
function injectHighlights(resolvedText,hl){ if(!resolvedText) return ''; let html=String(resolvedText); (hl||[]).forEach(h=>{ const safe=h.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'); const rx=new RegExp(`(\\b${safe}\\b)`,'ig'); html=html.replace(rx,'<span class="ink ink--hot">$1<\/span>'); }); return html; }

/* ===================== NOT FOUND ===================== */
function showNotFound(){ nav?.classList.add('show'); if(navTimer) navTimer.style.display='none'; stage?.classList.remove('show'); endCta?.classList.add('hidden'); brandMarquee?.classList.remove('show'); if(hintOverlay){ hintOverlay.classList.add('hide'); hintOverlay.style.display='none'; } skipIntroBtn?.classList.remove('show'); skipIntroBtn?.classList.add('hidden'); if(sceneHello) sceneHello.classList.add('hidden'); if(sceneGoal) sceneGoal.classList.add('hidden'); if(sceneRecorded){ const h2=sceneRecorded.querySelector('.subbig'); if(h2) h2.innerHTML = `This presentation <span class="ink ink--hot">doesn’t<\/span> exist for this link.`; const note=sceneRecorded.querySelector('.note'); if(note) note.innerHTML = `Try the <a href="/" class="ink ink--hot">link<\/a> we sent again.`; sceneRecorded.classList.remove('hidden'); sceneRecorded.classList.add('fade-in'); } }
...
async function hydrateFromCloud(){
  ...
  try{
    ...
    let linkQ = SB.from('hello_links')
      ...
      .eq('slug', slugFromQS).eq('active', true).limit(1);
    ...
    function gatherVideoCandidates(v,kind,buckets){ if(!v) return []; const out=[]; const bucket=kind==='slides'?buckets.slides:buckets.cam; const bases=[kind, kind==='webcam'?'cam':'deck', 'primary','main','video']; for(const base of bases){ for(const k of [base+'Id', base+'_id', base+'ID']) if(v[k]) out.push({id:v[k]}); for(const k of [base+'Key',base+'_key',base+'Path',base+'_path',base+'StorageKey']) if(v[k]) out.push({bucket, path:v[k]}); for(const k of [base+'Url',base+'_url',base+'URL']) if(v[k]) out.push({url:v[k]}); if(v[base] && typeof v[base]==='object'){ const o=v[base]; if(o.id) out.push({id:o.id}); if(o.bucket&&(o.key||o.path)) out.push({bucket:o.bucket, path:o.key||o.path}); if(o.key) out.push({bucket, path:o.key}); if(o.path) out.push({bucket, path:o.path}); if(o.url) out.push({url:o.url}); } } return out; }
    function gatherRootVideoCandidates(obj,kind,buckets){ if(!obj) return []; const bkt=kind==='slides'?buckets.slides:buckets.cam; const keys=[`${kind}Key`,`${kind}_key`,`${kind}Path`,`${kind}_path`,`${kind}Url`,`${kind}_url`,`${kind}URL`,`${kind}Id`,`${kind}_id`,`${kind}ID`, kind==='slides'?'deckKey':'camKey', kind==='slides'?'deckUrl':'camUrl', kind==='slides'?'deckId':'camId', 'videoUrl','video_key','videoKey','video_path','videoPath','videoId','mainVideo','primaryVideo']; const out=[]; for(const k of keys){ const v=obj?.[k]; if(!v) continue; if(k.toLowerCase().endsWith('id')) out.push({id:v}); else if(k.toLowerCase().includes('url')) out.push({url:v}); else out.push({bucket:bkt, path:v}); } return out; }

    const slidesCandidates=[ ...gatherVideoCandidates(vOver,'slides',BKT), ...gatherRootVideoCandidates(link?.overrides,'slides',BKT), ...gatherVideoCandidates(vDef,'slides',BKT), ...gatherRootVideoCandidates(pres,'slides',BKT), legacySlidesUrlOverride?{url:legacySlidesUrlOverride}:null, legacySlidesUrlDefault?{url:legacySlidesUrlDefault}:null ].filter(Boolean);
    const webcamCandidates=[ ...gatherVideoCandidates(vOver,'webcam',BKT), ...gatherRootVideoCandidates(link?.overrides,'webcam',BKT), ...gatherVideoCandidates(vDef,'webcam',BKT), ...gatherRootVideoCandidates(pres,'webcam',BKT), legacyWebcamUrlOverride?{url:legacyWebcamUrlOverride}:null, legacyWebcamUrlDefault?{url:legacyWebcamUrlDefault}:null ].filter(Boolean);

    const [slidesUrl, webcamUrl] = await Promise.all([
      pickFirstVideoURLAsync(slidesCandidates,BKT.slides),
      pickFirstVideoURLAsync(webcamCandidates,BKT.cam)
    ]);

    // Handcam (optional)
    const handCandidates=[]; const handUrl = await pickFirstStorageURLAsync(handCandidates);

    // CTA resolve (includes riskline)
    const baseCTA = pres?.cta || {}; const overCTA = (link?.overrides?.cta) || {}; const cta = { ...baseCTA, ...overCTA };
    const ctaResolved = {
    ...
    // Load media + auto-resign if signed URLs expire
    try{
    ...
    const hue = link?.overrides?.theme?.hue ?? pres?.theme?.hue; if (typeof hue === 'number') document.documentElement.style.setProperty('--hue', String(hue));

    PIN_REQUIRED = !!link?.require_pin;
    PIN_EXPECTED = (link?.settings?.pin_plain || '').trim() || TEST_PIN;
    __PIN_EXPECTED_LEN = Math.max(1, String(PIN_EXPECTED).length || 4);
    ...
    // After hydration, clamp the PIN max length to the true expected length
    if (window.__setPinMax) window.__setPinMax(__PIN_EXPECTED_LEN);

  }catch(err){ console.warn('Hydrate failed:', err); HYDR.notFound=true; PIN_REQUIRED=false; _resolveHydrate(); }
}
hydrateFromCloud();
if(!slugFromQS){ hideHeadlineTimer(); }

/* ===================== Intro & stage (kept) ===================== */
const SLIDE_DURATION_MS=1800; const SHIFT_UP_PX=110; const SHIFT_RIGHT_PX=24; const SCALE_TOP=1.2; const FLOAT_RADIUS_X=6; const FLOAT_RADIUS_Y=8; const FLOAT_SPEED_X=.9; const FLOAT_SPEED_Y=.75; const FINGERPRINT_GIF_MS=1800; const FINGERPRINT_FADE_MS=450; const STORAGE_KEY_UNLOCK='pv_seek_unlocked_after_unmuted_watch'; const STORAGE_KEY_INTRO_OK='pv_intro_watched_once'; const SEEK_REVERT_EPS=.75; let ignoreSeeksUntil=0;
const introWatched = localStorage.getItem(STORAGE_KEY_INTRO_OK)==='1';
let introActive=false; const introTimers=[]; const schedule=(fn,delay)=>{ const id=setTimeout(fn,delay); introTimers.push(id); return id; }; function cancelIntroSequence(){ introActive=false; introTimers.forEach(clearTimeout); introTimers.length=0; [sceneHello,sceneRecorded,sceneGoal].forEach(el=>{ if(!el) return; el.classList.add('hidden'); el.classList.remove('fade-in','fade-out'); }); }
...
function runSequence(){ introActive=true; if(introWatched) skipIntroBtn.classList.add('show'); schedule(()=>{ if(!introActive) return; show(sceneHello); try{ sfxHello.currentTime=0; sfxHello.play().catch(()=>{}); }catch{} },300); const fingerGif=document.getElementById('fingerGif'); if(fingerGif){ schedule(()=>{ if(!introActive) return; fingerGif.classList.add('is-fading'); setTimeout(()=>{ fingerGif.style.display='none'; },450); },1800); } schedule(()=>{ if(!introActive) return; hide(sceneHello); },2600); schedule(()=>{ if(!introActive) return; show(sceneRecorded); },3000); schedule(()=>{ if(!introActive) return; hide(sceneRecorded); },5600); schedule(()=>{ if(!introActive) return; show(sceneGoal); },5800); schedule(()=>{ if(!introActive) return; ctaSlide&&ctaSlide.classList.add('show'); },7200); schedule(()=>{ if(!introActive) return; hide(sceneGoal); },8200); schedule(()=>{ if(!introActive) return; nav.classList.add('show'); showStage(); localStorage.setItem(STORAGE_KEY_INTRO_OK,'1'); skipIntroBtn.classList.add('show'); },8400); }

/* ===================== Unmute + bind videos ===================== */
let hasClickedUnmuteOverlay=false; let unmutedSessionStartedAtZero=false; let programmaticSeekOK=false; function setCurrentTimeSafely(t){ programmaticSeekOK=true; try{ mainVideo.currentTime=t; } finally { setTimeout(()=>{ programmaticSeekOK=false; },0); } }
...
function isPrimaryWebcamActive(){ const op1=parseFloat(getComputedStyle(webcamVideo).opacity||'1'); const op2=parseFloat(getComputedStyle(webcamVideo2).opacity||'0'); return !isInEndMode && op1>=.5 && op2<.5; }
...
hintOverlay.addEventListener('click',unmuteAndRestartFromZero); hintBtn.addEventListener('click',(e)=>{ e.stopPropagation(); unmuteAndRestartFromZero(); });
...
let goEndWrap, goEndBtn; let seekUnlocked = localStorage.getItem(STORAGE_KEY_UNLOCK)==='1';
function isSeekEnabled(){ return seekUnlocked; }
function applySeekLockUI(){ if(isSeekEnabled()){ progress.classList.remove('disabled'); progress.setAttribute('aria-disabled','false'); } else { progress.classList.add('disabled'); progress.setAttribute('aria-disabled','true'); } if(goEndWrap){ goEndWrap.classList.add('visible'); const inEnd=playerEl.classList.contains('content-hidden'); const btn=goEndWrap.querySelector('.go-end-btn'); if(btn) btn.style.display=(isSeekEnabled()&&!inEnd)?'':''; } }
...
function seekFromEvent(e){ if(!isSeekEnabled()) return; if(!isFinite(mainVideo.duration)||mainVideo.duration<=0) return; const rect=progress.getBoundingClientRect(); const clientX=(e.clientX!==undefined)?e.clientX:(e.touches&&e.touches[0]?e.touches[0].clientX:0); const x=Math.min(Math.max(0, clientX-rect.left), rect.width); const ratio=x/rect.width; const newTime=ratio*mainVideo.duration; setMainTimeAndHardSync(newTime); }
progress.addEventListener('click',seekFromEvent); progress.addEventListener('touchstart',seekFromEvent,{passive:true});
...
let previewLoopActive=true; const PREVIEW_LOOP_EPS=.12; mainVideo.addEventListener('timeupdate',()=>{ if(!previewLoopActive||hasClickedUnmuteOverlay) return; const d=mainVideo.duration; if(!isFinite(d)||d<=0) return; if(mainVideo.currentTime>=d-PREVIEW_LOOP_EPS){ setMainTimeAndHardSync(.03); ignoreSeeksUntil=performance.now()+200; if(mainVideo.paused) mainVideo.play().catch(()=>{}); } });
...
/* ===================== Slide mechanics / orbit / end state (kept) ===================== */
...
mainVideo.addEventListener('ended',()=>{ if(!hasClickedUnmuteOverlay){ setMainTimeAndHardSync(.03); ignoreSeeksUntil=performance.now()+200; mainVideo.play().catch(()=>{}); return; } maybeUnlockSeekOnEnd(); frameEl.classList.add('dim'); playerEl.classList.add('content-hidden'); navTimer?.classList.add('hide'); slideWebcamUpSmall(); showEndUI(); crossfadeToWebcamEnd(300); });
...
/* ===================== PIN overlay logic (REWORKED for mobile) ===================== */
(function setupPin(){
  const pinOverlay = document.getElementById('pinOverlay');
  const pinBoxes   = document.getElementById('pinBoxes');
  const pinError   = document.getElementById('pinError');
  const cells = Array.from(pinBoxes ? pinBoxes.querySelectorAll('.pin-digit') : []);

  // Invisible numeric input – single source of truth
  const pinInput = document.createElement('input');
  pinInput.type = 'tel';
  pinInput.inputMode = 'numeric';
  pinInput.pattern = '[0-9]*';
  pinInput.autocomplete = 'one-time-code';
  pinInput.maxLength = 10; // generous until hydration tells us real length
  ...
  if (pinOverlay) pinOverlay.appendChild(pinInput);

  // Allow hydration to shrink max later
  window.__setPinMax = (n)=>{ const m=Math.max(1, Math.min(10, n|0)); pinInput.maxLength=m; };

  function focusPinSoon(){ setTimeout(()=>{ try{ pinInput.focus(); }catch{} }, 0); }
  pinOverlay?.addEventListener('click', focusPinSoon);

  // Normalize ANY numerals to ASCII 0-9 and drop anything else
  function normalizeDigits(str){
    let out='';
    for(const ch of String(str||'')){
      const code = ch.codePointAt(0);
      // ASCII 0-9
      if(code>=0x30 && code<=0x39){ out += ch; continue; }
      ...
      // Ignore spaces, dashes and other fillers
    }
    return out;
  }

  function renderFromInput(){
    const raw = normalizeDigits(pinInput.value).slice(0, pinInput.maxLength);
    // mirror into boxes
    cells.forEach((c, i)=>{
      const filled = i < raw.length;
  ...
  function flashError(){ if(!pinError) return; pinError.classList.add('show'); cells.forEach(c=>{ c.classList.add('pulse'); setTimeout(()=>c.classList.remove('pulse'), 900); }); setTimeout(()=> pinError.classList.remove('show'), 1400); }
  function clearAll(){ pinInput.value=''; renderFromInput(); }
  ...
  async function checkIfReady(){
    // If PIN not required, continue immediately
    if(!PIN_REQUIRED){ startAfterPin(); return; }
    const attempt  = normalizeDigits(pinInput.value);
    const expected = normalizeDigits(PIN_EXPECTED);

    // Tighten max to expected length once known
    if(expected && expected.length){ window.__setPinMax(expected.length); }

    if(attempt.length < (expected.length||4)) return; // wait until full length
    if(attempt === expected){ startAfterPin(); }
    else { flashError(); clearAll(); }
  }
  ...
  // Input events (works with phone virtual keyboard & OTP autofill)
  pinInput.addEventListener('input', ()=>{ renderFromInput(); playPinSound(); checkIfReady(); });

  // Desktop typing via DevTools remote debugging: push into the input, not elsewhere
  function onDocKey(e){
    if(!pinOverlay || pinOverlay.style.display==='none') return;
    const k=e.key;
    // Ignore typing when focused in another field
    const t=e.target; const isTextField = t && (t.tagName==='INPUT' || t.isContentEditable);
    if(isTextField && t!==pinInput) return;

    if(k>='0' && k<='9'){
      pinInput.value = (normalizeDigits(pinInput.value) + k).slice(0, pinInput.maxLength);
      renderFromInput(); playPinSound(); checkIfReady(); e.preventDefault(); return;
    }
    if(k==='Backspace' || k==='Delete'){
      const cur = normalizeDigits(pinInput.value);
      pinInput.value = cur.slice(0, Math.max(0, cur.length-1));
      renderFromInput(); playPinSound(); e.preventDefault(); return;
    }
  }
  document.addEventListener('keydown', onDocKey, true);

  // Initial draw and focus
  renderFromInput(); focusPinSoon();

  (async function waitForPinDecision(){ await HYDRATE_DONE; if(!PIN_REQUIRED){ startAfterPin(); return; } __PIN_EXPECTED_LEN = Math.max(1, String(PIN_EXPECTED||'').length || 4); window.__setPinMax(__PIN_EXPECTED_LEN); })();
})();

/* ===================== Music toggle ===================== */
(function setupMusicToggle(){ if(!musicToggle) return; if(bgMusic) setVolumeSafe(bgMusic,MUSIC.MAIN_VOL); if(endMusic) setVolumeSafe(endMusic,0); function updateBtn(){ const off=!musicEnabled||((bgMusic?.paused??true)&&(endMusic?.paused??true)); musicToggle.classList.toggle('off',off); } musicToggle.addEventListener('click',()=>{ musicEnabled=!musicEnabled; if(!musicEnabled){ pauseBothMusic(); } else { playActiveMusicForState(); } updateBtn(); }); bgMusic?.addEventListener('play',updateBtn); bgMusic?.addEventListener('pause',updateBtn); endMusic?.addEventListener('play',updateBtn); endMusic?.addEventListener('pause',updateBtn); updateBtn(); })();

/* ===================== Seek-locked message spacing + z-index + width ===================== */
function applySeekInsightStyleFromCSS(){ if(!seekInsight) return; const cs=getComputedStyle(document.documentElement); const gap=(cs.getPropertyValue('--seekinsight-gap')||'').trim()||'10px'; const z=(cs.getPropertyValue('--seekinsight-z')||'').trim()||'10060'; const w=(cs.getPropertyValue('--seekinsight-width')||'').trim(); seekInsight.style.marginTop=gap; seekInsight.style.zIndex=String(parseInt(z,10)||10060); if(w){ seekInsight.style.maxWidth=w; seekInsight.style.width='auto'; } }
...
function setupGoToEndButton(){ if(!playerEl) return; goEndWrap=document.createElement('div'); goEndWrap.className='go-end-wrap'; goEndBtn=document.createElement('button'); goEndBtn.id='goToEndBtn'; goEndBtn.type='button'; goEndBtn.className='go-end-btn'; goEndBtn.setAttribute('aria-label','Go to the end'); goEndBtn.innerHTML=`\n    <i class="ri-skip-forward-fill" aria-hidden="true"></i>\n    <span class="go-end-text">Go to the end</span>\n  `; goEndWrap.appendChild(goEndBtn); playerEl.parentNode.insertBefore(goEndWrap, playerEl.nextSibling); goEndBtn.addEventListener('click',()=>{ if(goEndWrap){ const btn=goEndWrap.querySelector('.go-end-btn'); if(btn) btn.style.display='none'; } goToEndNow(); }); const btn=goEndWrap.querySelector('.go-end-btn'); if(btn) btn.style.display='none'; window.__belowPlayerArea = goEndWrap; function relocateWebcamForSmall(){ const isSmall=window.matchMedia('(max-width: 768px)').matches; if(!webcamWrap) return; if(isSmall && document.body.classList.contains('offer-mode') && topDock){ if(webcamWrap.parentNode!==topDock) topDock.appendChild(webcamWrap); document.body.dataset.webcamPosition='below'; requestAnimationFrame(()=>{ positionOrbitOnRim(315); }); return; }
  ...
  window.__relocateWebcamForSmall = relocateWebcamForSmall; relocateWebcamForSmall(); window.addEventListener('resize', relocateWebcamForSmall); applySeekLockUI(); }
setupGoToEndButton();

/* ===================== Hover time label + bigger hitbox (kept) ===================== */
(function setupHoverTimeLabel(){ if(!progress||!mainVideo) return; const HITBOX_PAD=16; const SCRUB_START_PX=4; function getLabelGap(){ const cs=getComputedStyle(document.documentElement); const v=(cs.getPropertyValue('--seeklabel-gap')||'').trim(); const px=parseFloat(v); return Number.isFinite(px)?px:8; } const clamp=(n,a,b)=>Math.max(a,Math.min(b,n)); const fmtClock=(sec)=>{ sec=Math.max(0,Math.floor(sec||0)); const h=Math.floor(sec/3600), m=Math.floor((sec%3600)/60), s=sec%60; const pad=n=>String(n).padStart(2,'0'); return h>0?`${h}:${pad(m)}:${pad(s)}`:`${m}:${pad(s)}`; }; const rect=()=>progress.getBoundingClientRect(); const duration=()=> (isFinite(mainVideo.duration)?mainVideo.duration:0); const timeFromClientX=(clientX)=>{ const r=rect(); const x=clamp(clientX - r.left, 0, r.width); const ratio=r.width? x/r.width: 0; return ratio*duration(); };
